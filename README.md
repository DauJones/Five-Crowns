...
Scores are saved in **localStorage** — a small built-in storage area every browser provides. Think of it like a sticky note the browser keeps on behalf of the page.

- **Device-specific** — only on the device where the game was played
- **Browser-specific** — Chrome and Safari each keep their own separate data
- **Permanent until cleared** — stays until someone starts a new game, or manually clears their browser data
...
Open `index.html` in any web browser. No build step or installation required.
